...
pub async fn start() {
    // 加载配置文件
    let config = match Config::load_with_config("config/feature-config.yml")  {
        Ok(data) => data,
        Err(err) => {
            error!("{}", err);
            return;
    ...
    // 打印服务器信息
    println!("Starting Feature Server: http://{}:{}", &server.host, &server.port);
    println!(
        "Starting Feature Server: http://{}:{}",
        get_local_ip().unwrap(),
        &server.port
    );
    // 路由
